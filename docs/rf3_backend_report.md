# RF3 / RFdiffusion All-Atom Backend Report

Date: 2026-06-28

## Naming Decision

The deployable upstream found and installed for this stage is
`baker-laboratory/rf_diffusion_all_atom`, whose README names the code
"RFDiffusion AA". A separate public upstream repository or release explicitly
named "RFdiffusion3" was not confirmed during this pass.

This stack therefore uses the backend name:

```text
rfdiffusion_all_atom
```

For operator convenience, `BACKBONE_BACKEND=rf3` is accepted as an alias in the
launcher, but logs and reports normalize the name to `rfdiffusion_all_atom`.
Do not describe this as a replacement for RFdiffusion v1.
...
Environment note: ~/protein_design/envs/rfdiffusion_all_atom.env
...
```text
RF3_ALLOW_UNIDEALIZED_FALLBACK=1
```
...
```bash
cd ~/protein_design/examples/epitope_scaffold/rfdiffusion_all_atom_smoke_5tpn
...
rfdiffusion_all_atom -> ProteinMPNN -> AF3 -> filter -> merge
```
...
```text
Run directory: /public/home/yinyifan/protein_design/examples/epitope_scaffold/rfdiffusion_all_atom_smoke_5tpn_20260628_194145
Slurm job: 123174
Node: gpu14
State: COMPLETED
Elapsed: 00:01:52
...
rfdiffusion_all_atom: job 123193_6 FAILED, retried as 123254_6 and completed
...
| rfdiffusion_all_atom | 10/10 | 10/10 | 10/10 | 0/10 | 88.8509 | 7.0484 | 6.5673 | 0.0000 | design_8 | FAIL |
...
```text
examples/epitope_scaffold/rfdiffusion_all_atom_smoke_5tpn_20260628/
